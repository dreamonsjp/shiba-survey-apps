...
var Application = {
    onCreate: function() {
        ...
        document.addEventListener('pagebeforecreate', this.updateIcons, false);

        document.addEventListener('deviceready', this.initApplication, false);
    },
    initApplication: function() {
        ...
        $(document)
                ...
                .on('pageinit', '#detailsurvey', function() {
                    ...
                    var sid = this.getAttribute('data-url').replace(/(.*?)sid=/g, '');
                    g_selectedSid = sid;
                    Application.initDetailSurvey(sid);
        ...
        $('#menupage').on("swipeleft", Application.swipeActionHandling);
    ...
    initPreviewPage: function(sid)
    {
        ...
        var withText = true;
        if (String(uaTemp).indexOf("iPhone") >= 0)
            withText = false;
        var htmlContent = '';
    ...
    previewMenuItemHandling: function()
    {
        ...
        $(document).on("click", "a.SendSavedData", function() {
            ...
            confirmDialog("Send Survey", "Are you sure you want to send No." + $(this).children(".idPos") + "?", function() {


                var savedCode = $(this).attr('data-savedCode');
                ...
                if (Application.isConnection())
                {
                    ...
                    if (savedData)
                    {
                        ...
                        var $workingItem = $(this).parent().parent().parent().parent();


                        $.mobile.loading('show', {
                            text: 'Please wait while your saved is being sent!',
                            textVisible: true,
                            textonly: false
                        });
                        ...
                        $.ajax({
                            ...
                            success: function(data)
                            {
                                ...
                                $workingItem.nextAll().each(function() {

                                    var currentIndex = parseInt($(this).children(".idPos").html());
                                    currentIndex -= 1;
                                    $(this).children(".idPos").empty();
                                    $(this).children(".idPos").html(currentIndex);
                                });
        ...
        $(document).on("click", "a.DeleteSavedData", function() {
            ...
            confirmDialog("Delete Survey", "Are you sure you want to delete No." + $(this).children(".idPos") + "?", function() {
    ...
    uploadAFile: function(inputVal, fileURI, surveyID, surveyLang, fieldName)
    {
        ...
        ft.upload(fileURI, feedUrl + "/upload.php", function(message) {
            ...
            {
                var htmlVal = '[{"title":"","comment":"","size":"' + returnValue.size + '","name":"' + returnValue.name + '","filename":"' + returnValue.filename + '","ext":"' + returnValue.ext + '"}]';
    ...
    menuPageButtonHandling: function()
    {
        ...
        $(document).on("click", '#menupreview', function() {
            ...
            if (listSavedData)
                ...
                navigator.notification.alert('There is no saved data for this survey!', function() {
                }, 'Error');
        ...
        $(document).on("click", '#menusend', function() {
            ...
            var totalSaved = Application.getNumSavedData(g_selectedSid, g_username);
            var numSend = 0;
            if (totalSaved <= 0)
                return;
            ...
            if (Application.isConnection())
            {
                ...
                $.mobile.loading('show', {
                    text: 'Please wait while your saved surveys are being sent!',
                    textVisible: true,
                    textonly: true
                });
                ...
                setTimeout(function() {
                    ...
                    console.log('send all saved data of ' + g_selectedSid + ' username ' + g_username);
                    var listSavedData = window.localStorage.getItem('ShibaSavedList_' + sid + "_" + g_username);
                    ...
                    if (listSavedData)
                    {
                       ...
                       data: savedData.data,
                                        ...
                                        success: function(data)
                                        {
                                            ...
                                            Application.deleteARecord(g_selectedSid, g_username, aSaved.savedcode);
                                        },
                                        complete: function()
                                        {
                                            loopWaiting = false;
                    ...
                    if (numSend == totalSaved - 1)
                        alert('All your saved data has been submitted successfully.');
                    else
                        alert(numSend + ' your saved data has been submitted successfully.');
    ...
    sendASavedData: function(saveCode)
    {
    ...
    sendAllSavedData: function(sid, username)
    {
        if (Application.isConnection())
        {
            console.log('send all saved data of ' + sid + ' username ' + username);
            var listSavedData = window.localStorage.getItem('ShibaSavedList_' + sid + "_" + username);
            if (listSavedData)
            {
                listSavedData = JSON.parse(listSavedData);
                ...
                {
                    var aSaved = eval('(' + JSON.stringify(listSavedData[index]) + ')');
                    Application.sendASavedData(aSaved.savedcode);
                }
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (empObject.result == "SUCCESS" && empObject.code == 118)
                    {
                        ...
                        //Application.layoutDetailSurvey(empObject.posts,sid);
                        setTimeout(function() {
                            $.mobile.loading('hide');
                        }, 2000);
    ...
    layoutDetailSurvey: function(returnData, sid, jscode, relevance) {
        ...
        if (listLanguage)
        {
            ...
            if (listLang.length >= 1)
            {
                ...
                htmlContent += '<option value="en">English</option>';
                for (var index = 0; index < listLang.length; index++)
                {
                    htmlContent += '<option value="' + listLang[index] + '">';
        ...
        $detailContent.children('#langSwitching').children('#langListing').on('change', function() {

            g_selectedSurveyLang = $(this).val();

            initDetailSurvey(g_selectedSid, g_selectedSurveyLang);
        ...
        if (g_newdata == false)
        {
            $('#revertingpopup').popup('open', {history: false});

            Application.editASavedData(g_savedCode);

            $('#revertingpopup').popup('close');
        }
    ...
    listPageTopButtonHandling: function() {
        ...
        $(document).on('click', "#refreshButtonList", function() {
            ...
            {
                $.mobile.loading('show', {
                    text: 'Please wait while refreshing the list of survey!',
                    textVisible: true,
                    textonly: true
                });
    ...
    surveySubmitHandling: function()
    {
        $('#surveySubmit').submit(function(event)
        {
            ...
            if (numMandatoryNotFilled != 0)
            {
                ...
                navigator.notification.alert('One or more mandatory questions have not been answered. You cannot proceed until these have been completed.', function() {
                }, 'Error');
            }
            else
            {
                ...
                console.log(JSON.stringify(getDataOriginal));

                if (g_newdata)
                    Application.saveANewRecord(g_selectedSid, g_username, getData, getDataOriginal, g_selectedSurveyLang);
                else
                    Application.updateARecord(g_selectedSid, g_username, g_savedCode, getData, getDataOriginal);
    ...
    saveANewRecord: function(sid, username, data, originalData, language)
    {
        var savedCode = Application.saveACode(sid, username);
        var aSavedData = {"lang": language, "sid": sid, "data": data, "original": originalData};
        window.localStorage.setItem(savedCode, JSON.stringify(aSavedData));

    },
    updateARecord: function(sid, username, savedCode, data, originalData)
    {
        var savedData = window.localStorage.getItem(savedCode);
        savedData = JSON.parse(savedData);
        savedData.data = data;
            ...
            case "|":
                {
                    ...
                    htmlItems += '<div class="ui-body ui-body-a wrapContent">';
                    htmlItems += '<input type="afile" class="' + (anQuestion.mandatory == "Y" ? "anMandatory" : "") + ' fileType" onclick="getAFile(this);" readonly="true" data-clear-btn="true" name="' + sid + "X" + anQuestion.gid + "X" + anQuestion.qid + '" id="answer' + sid + "X" + anQuestion.gid + "X" + anQuestion.qid + '" value="">';
            ...
            case "R":
                {
                    ...
                    for (var index = 0; index < length; index++)
                    {
                        ...
                        htmlItems += '<input type="hidden" name="' + sid + "X" + anQuestion.gid + "X" + anQuestion.qid + "_SHIBASURVEYRANK_" + answerDetail.code + '" value="' + index + '" class="rankingInputOrder">';
    ...
    initListPage: function() {
        ...
        if (surveyList == null || g_forceRefreshListSurvey == true)
        {
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    $.mobile.loading('show', {
                        text: 'Please wait while retrieving the list of survey...',
                        textVisible: true,
                        textonly: false
                    });
                },
    ...
    getLanguageList: function(sid) {
        ...
        var surveyList = JSON.parse(window.localStorage.getItem('surveyList'));
        ...
        $('#loginForm').submit(function(event) {
            ...
            else
            {
                ...
                switch (loginOfflineStatus)
                {
                    ...
                    case 1:
                        {
                            navigator.notification.alert('Your password is invalid for offline mode.Please try again!', function() {
                            }, 'Error');
                        }
                        break;
                    case 0:
                    case -1:
                        {
                            navigator.notification.alert('Your username is invalid for offline mode.Please try again later', function() {
                            }, 'Error');
                        }
                        break;
                }
